...
struct _config
{
	...
	static constexpr int runs_default = 1;
...
void usage(ostream& os, const char* const program_name, int exit_code)
{
	os << "USAGE:\n"
		...
		<< "\t-r|--runs     \tRun all benchmarks requested number of times\n"
		...
		<< "EXAMPLE:\n"
		<< program_name << " -o -r 3 --num-max 28 --thread-min 12 --series-name my_shorter_test"
		<< endl;
...
int throwing_main(int argc, const char* argv[])
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		else if (streq(argv[i], "-r") || streq(argv[i], "--runs"))
		{
			config.runs = arg_to_int(argv[i], argv[i + 1], 1, INT32_MAX);
			i++;
		}
	...
	{
		if (config.runs != 1)
			cout << tabs++ << "Executing benchmark run " << run << " out of " << config.runs << endl;

		cout << tabs++ << "Measuring throughput..." << endl;
		bool warmup = true;
		...
		cout << tabs++ << "Performing multi-linear benchmarks..." << endl;
		warmup = true;
